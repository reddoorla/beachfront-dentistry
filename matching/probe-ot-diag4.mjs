...
const probe = () => {
  ...
  const hh = isLive
    ? document.querySelector(".meet-heading")
    : document.querySelector('[data-slice-variation="subpage"] h2');
  out.heroHeadingBox = R(hh);
  if (hh) {
    const s = getComputedStyle(hh);
    out.heroHeadingStyle = {
      fs: s.fontSize,
      lh: s.lineHeight,
      fw: s.fontWeight,
      ta: s.textAlign,
    ...
    const rects = [...rg.getClientRects()].map((r) => ({
      x: +r.left.toFixed(1),
      y: +(r.top + window.scrollY).toFixed(1),
      w: +r.width.toFixed(1),
      h: +r.height.toFixed(1),
  ...
  out.heroBand = R(
    isLive
      ? document.querySelector(".hero.redondo")
      : document.querySelector('[data-slice-variation="subpage"]'),
  );
  ...
  if (card) {
    ...
    if (isLive) {
      ...
      out.rmArrow = R(arrow);
      if (arrow) {
        const s = getComputedStyle(arrow);
        out.rmArrowStyle = {
          w: s.width,
          h: s.height,
          m: s.margin,
          ...
          filter: s.filter,
          nat: arrow.naturalWidth + "x" + arrow.naturalHeight,
          src: arrow.currentSrc,
        };
      }
      ...
      if (p) {
        ...
        out.bioStyle = {
          ...
          lh: s.lineHeight,
          m: s.margin,
          wc: s.webkitLineClamp,
        };
        out.bioText = p.innerText;
        out.bioLines = [
          ...(() => {
            const rg = document.createRange();
      ...
      const a = [...card.querySelectorAll("a")].find((x) =>
        /read more/i.test(x.textContent || ""),
      );
      out.rmDiv = R(a);
      if (a) {
      ...
      const tn =
        a &&
        [...a.childNodes].find((n) => n.nodeType === 3 && n.textContent.trim());
      if (tn) {
        const rg = document.createRange();
        rg.selectNode(tn);
        const r = rg.getBoundingClientRect();
      ...
      if (arrow) {
        ...
        out.rmArrowStyle = {
          ...
          mask: s.webkitMaskImage.slice(0, 60),
        };
      }
  ...
  const grid = document.querySelector(
    isLive ? ".team-grid-section" : ".team-grid-section",
  );
  out.gridBox = R(grid);
  if (isLive) {
    ...
    const h2 = [...document.querySelectorAll("h1,h2,h3")].find((e) =>
      /ready for great/i.test(e.textContent || ""),
    );
    out.readyHeading = R(h2);
    if (h2) {
      const s = getComputedStyle(h2);
      out.readyStyle = { fs: s.fontSize, lh: s.lineHeight, ta: s.textAlign };
    }
  } else {
    const h2 = [...document.querySelectorAll("h1,h2,h3")].find((e) =>
      /ready for great/i.test(e.textContent || ""),
  ...
  for (const [name, url] of [
    ...
    for (const vw of VPS) {
      ...
      if (vw === 1440) {
        ...
        if (card)
          await card.screenshot({ path: `matching/ot-card-${name}-1440.png` });
      }
      await ctx.close();
    }
  }
