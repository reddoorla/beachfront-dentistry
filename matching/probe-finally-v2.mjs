...
async function measure(p) {
  return p.evaluate(() => {
    ...
    const next = [...document.querySelectorAll("*")].find(
      (e) =>
    ...
    const cards = [...document.querySelectorAll("*")]
      ...
      .filter((o) => {
        ...
        return (
          ...
          o.r.height <= 540 &&
          o.r.width >= 250 &&
          o.r.top + scrollY > y(hb)
        );
      })
      .map((o) => ({
        top: y(o.r),
        bottom: Math.round(o.r.bottom + scrollY),
    ...
    for (const c of cards)
      if (!uniq.some((u) => Math.abs(u.top - c.top) < 20)) uniq.push(c);
    return {
  ...
  for (const vw of VWS) {
    ...
    for (const [k, url] of Object.entries(TARGETS)) {
      ...
      try {
        ...
        const c = m.cards;
        const gap = (i) =>
          c[i] && c[i + 1] ? c[i + 1].top - c[i].bottom : "?";
        console.log(
          `  ${k}: hBot=${m.headingBottom}  cards=[${c.map((x) => `${x.top}-${x.bottom}(h${x.h})`).join(", ")}]`,
        );
        console.log(
          `      hdr→c1=${c[0] ? c[0].top - m.headingBottom : "?"}  gap1=${gap(0)}  gap2=${gap(1)}  c3→next=${c[2] && m.nextTop ? m.nextTop - c[2].bottom : "?"}`,
        );
      } catch (e) {
