...
for (const [tag,url,sel] of [["REF","https://www.beachfrontdentistry.com/",".big-teal-tooth"],["CAND","http://localhost:5190/",'img[src*="big-teal-tooth"]']]){
 for (const w of WIDTHS){
  ...
  const r=await p.evaluate((sel)=>{
    ...
    if(!t){t=[...document.querySelectorAll("img")].find(e=>/big-teal-tooth|tooth/i.test(e.src||"")&&e.getBoundingClientRect().width>40);}
    if(!t)return"MISS";
    const tr=t.getBoundingClientRect();
    // SERVICES heading as section landmark
    const svc=[...document.querySelectorAll("h1,h2,h3,h4,h5,h6,div,span,p")].find(e=>(e.textContent||"").trim().toUpperCase().startsWith("SERVICES")&&[...e.childNodes].some(n=>n.nodeType===3&&n.nodeValue.trim()));
    ...
    return {w:Math.round(tr.width),h:Math.round(tr.height),
      centerXpct:+(((tr.left+tr.width/2)/window.innerWidth)*100).toFixed(1),
      rightGapPct:+(((window.innerWidth-tr.right)/window.innerWidth)*100).toFixed(1),
