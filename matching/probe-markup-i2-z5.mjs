...
const found = await p.evaluate(() => {
  ...
  for (let step = 0; step < 200; step++) {
    ...
    for (const c of cards) {
      ...
      if (
        hr.bottom > shot.top + 10 &&
        hr.top < shot.bottom - 10 &&
        hr.right > shot.left + 4
      ) {
        return {
          y,
          header: { top: hr.top, bottom: hr.bottom, right: hr.right },
          shot: {
            top: shot.top,
            left: shot.left,
            right: shot.right,
...
if (!found) {
  console.log("no overlapping position found");
  await b.close();
  process.exit(0);
}
...
const state = await p.evaluate(() => {
  ...
  const card = cards[0];
  const inner =
    card.querySelector("[class*='rounded-[25px]']") || card.firstElementChild;
  const cs = (e) =>
    e
      ? {
          position: getComputedStyle(e).position,
          z: getComputedStyle(e).zIndex,
          transform: getComputedStyle(e).transform === "none" ? "none" : "set",
        }
      : null;
  return {
    cardsTotal: cards.length,
    cardsStillTransformed: withTransform,
    card: cs(card),
    cardInner: cs(inner),
    header: cs(card.querySelector("button")),
    wrapper: cs(
      document
        .querySelector(".ask-the-doctor-headshot")
...
const clip = {
  x: Math.round(found.shot.left),
  y: Math.round(Math.max(0, found.shot.top)),
  width: Math.round(found.shot.right - found.shot.left),
  height: Math.round(
    Math.min(900, found.shot.bottom) - Math.max(0, found.shot.top),
  ),
};
...
await p.evaluate(() => {
  document
    .querySelector(".ask-the-doctor-headshot")
    .closest("[class*='pointer-events-none']").style.zIndex = "30";
});
await p.waitForTimeout(150);
const fixed = await p.screenshot({ clip });

const A = PNG.sync.read(shipped),
  B = PNG.sync.read(fixed);
  ...
  JSON.stringify(
    {
      ...
      state,
      headerBandOnScreen: {
        top: +found.header.top.toFixed(1),
        bottom: +found.header.bottom.toFixed(1),
      },
      headshotBox: clip,
      changedPx: changed,
      pct: +((changed / (A.width * A.height)) * 100).toFixed(1),
      changedRowsWithBroadChange: solid.length
        ? `${solid[0].y}..${solid[solid.length - 1].y} (screen y ${clip.y + solid[0].y}..${clip.y + solid[solid.length - 1].y})`
        : "none",
      verdict:
        solid.length > 5
          ? "REAL OCCLUSION — a solid band changes when z is raised"
          : "no solid band; scattered pixels are AA only",
    },
    null,
    1,
  ),
