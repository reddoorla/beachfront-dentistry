...
(async () => {
  ...
  try {
    for (const w of [1440, 390]) {
      ...
      res[w] = await page.evaluate(() => {
        ...
        const meet = Array.from(hero.querySelectorAll("*"))
          .filter(isLeaf)
          .find((e) => /Meet/i.test(e.textContent));
        o.meet = meet
          ? { ...type(meet), text: meet.textContent.trim(), box: box(meet) }
          : null;
        ...
        const intro = Array.from(sub.querySelectorAll("*"))
          .filter(isLeaf)
          .find((e) => /We love caring/i.test(e.textContent));
        o.intro = intro
          ? {
              ...type(intro),
              text: intro.textContent.replace(/\s+/g, " ").trim().slice(0, 60),
              box: box(intro),
            }
          : null;
        // Our/Team leaves for confirm
        const ot = Array.from(sub.querySelectorAll("*"))
          .filter(isLeaf)
          .filter((e) => /^(Our|Team)$/.test(e.textContent.trim()))
          .map((e) => ({
            ...type(e),
  ...
  writeFileSync(
    "/Users/tuckerlemos/Documents/GitHub/beachfront-dentistry/matching/team-probe3.json",
