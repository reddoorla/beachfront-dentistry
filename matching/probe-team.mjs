...
async function probe(page, which) {
  return await page.evaluate(() => {
    ...
    const heads = Array.from(document.querySelectorAll("h1,h2,h3,span,div"))
      .filter((el) => {
    ...
    out.h1 = h1
      ? {
          text: h1.innerText.replace(/\s+/g, " ").trim().slice(0, 60),
          html: h1.innerHTML.slice(0, 200),
          ...type(h1),
          box: box(h1),
        }
      : null;
    ...
    if (h1) {
      ...
      while (p && p.parentElement && p.parentElement !== document.body) {
        ...
        if (
          ...
          p.parentElement.tagName === "SECTION" ||
          p.parentElement.tagName === "HEADER"
        ) {
          heroBand = p.parentElement;
          break;
        }
        p = p.parentElement;
      }
    ...
    if (h1) {
      ...
      for (const p of ps) {
        ...
        if (t.length > 30) {
          intro = { text: t.slice(0, 160), ...type(p), box: box(p) };
          break;
        }
      }
    }
    ...
    const candidates = Array.from(
      document.querySelectorAll("div,ul,section"),
    ).filter((el) => {
      const cs = getComputedStyle(el);
      return (
    ...
    for (const el of candidates) {
      ...
      const withHead = kids.filter((k) =>
        k.querySelector("h1,h2,h3,h4,h5"),
      ).length;
      const score = withImg + withHead;
      if (withImg >= 3 && score > bestScore) {
        bestScore = score;
        best = el;
      }
    }
    if (best) {
      ...
      const cardInfo = {
        ...
        boxShadow: ccs.boxShadow.slice(0, 80),
      ...
      const paras = Array.from(card.querySelectorAll("p")).map((p) => ({
        text: p.innerText.trim().slice(0, 80),
        ...type(p),
      }));
      cardInfo.paras = paras;
      // any button / read more
      const btns = Array.from(card.querySelectorAll("a,button")).map((b) => ({
      ...
      cardInfo.allImgs = Array.from(card.querySelectorAll("img")).map((im) => ({
        box: box(im),
        src: (im.currentSrc || im.src || "").slice(-50),
...
(async () => {
  ...
  writeFileSync(
    "/Users/tuckerlemos/Documents/GitHub/beachfront-dentistry/matching/team-probe.json",
