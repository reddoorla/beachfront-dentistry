...
try {
  ...
  if (!box) { console.log("no card"); } else {
    ...
    const info = await p.evaluate(() => {
      ...
      return { h: Math.round(r.height), readMoreBottomInCard: ar ? Math.round(ar.bottom - r.top) : null, readMoreVisible: ar ? (ar.bottom <= r.bottom + 1 && ar.top >= r.top - 1) : null };
    });
