...
const stack = async (url, label) => {
  ...
  const geo = await p.evaluate(() => {
    ...
    return {
      ...
      card: hit
        ? {
            top: +hit.b.top.toFixed(1),
            left: +hit.b.left.toFixed(1),
            right: +hit.b.right.toFixed(1),
            bottom: +hit.b.bottom.toFixed(1),
          }
        : null,
  ...
  if (geo.card) {
    ...
    if (x1 > x0 + 2 && y1 > y0 + 2) {
      ...
      const A = PNG.sync.read(withShot),
        B = PNG.sync.read(without);
      let d = 0;
      ...
      diffPx = {
        ...
        pct: +((d / (A.width * A.height)) * 100).toFixed(1),
      };
    }
  }
  await ctx.close();
