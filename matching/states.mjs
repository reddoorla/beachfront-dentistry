...
if (!page) {
  console.error(
    "usage: states.mjs <page> [--shot]   (page: a key in states/index.mjs)",
  );
  process.exit(2);
}
...
if (!def) {
  console.error(
    `no state list for "${page}". Have: ${Object.keys(PAGES).join(", ")}`,
  );
  process.exit(2);
}
  ...
  for (const st of def.states) {
    ...
    const near = (a, b) => {
      const x = Number.parseFloat(a),
        y = Number.parseFloat(b);
      return (
        Number.isFinite(x) && Number.isFinite(y) && Math.abs(x - y) <= 0.02
      );
    };
    ...
    if (diffs.length && DECLARED.some((d) => d.match(st.name, diffs, L.v))) {
      ...
      for (const k of diffs)
        console.log(
          `        (declared) ${k}: live "${L.v[k]}" vs ours "${O.v[k]}"`,
        );
      declared++;
      continue;
    }
    if (!diffs.length) {
      ...
      for (const k of info)
        console.log(
          `        (info) ${k}: live "${L.v[k]}" vs ours "${O.v[k]}"`,
        );
    } else {
      console.log(`FAIL  ${st.name}`);
      for (const k of diffs)
        console.log(`        ${k}: live "${L.v[k]}" vs ours "${O.v[k]}"`);
      for (const k of info)
        console.log(
          `        (info) ${k}: live "${L.v[k]}" vs ours "${O.v[k]}"`,
        );
