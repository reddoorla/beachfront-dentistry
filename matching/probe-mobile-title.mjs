...
const cyan = (c) =>
  /129, ?158, ?204|18, ?158, ?204/.test(c) || /#129ecc/i.test(c);
...
try {
  for (const [tag, base, path] of [
    ...
    const info = await p.evaluate((cyanSrc) => {
      ...
      const cs = (e) =>
        e
          ? `${getComputedStyle(e).fontSize}/${getComputedStyle(e).lineHeight}`
          : null;
      return {
        title: title
          ? `${cs(title)} top=${Y(title)} h=${H(title)} "${title.textContent.trim().slice(0, 22)}"`
          : null,
        lede: lede
          ? `${cs(lede)} top=${Y(lede)} h=${H(lede)} x=${Math.round(lede.getBoundingClientRect().left)} w=${Math.round(lede.getBoundingClientRect().width)}`
          : null,
