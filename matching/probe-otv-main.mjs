...
const measure = (side) => {
  ...
  const byText = (sel, needle) =>
    qa(sel).find((e) => (e.textContent || "").trim().startsWith(needle));
  ...
  const heroSec = live
    ? q("section.hero.redondo")
    : q('section[data-slice-variation="subpage"]');
  const meet = live ? q("h2.meet-heading") : heroSec ? q("h2", heroSec) : null;
  out.hero = R(heroSec);
  out.meetBox = R(meet);
  out.meetStyle = S(meet, [
    ...FONT,
    "position",
    "bottom",
    "left",
  ...
  out.meetWrapStyle = meet
    ? S(meet.parentElement, [
        "position",
        "bottom",
        "left",
        "width",
  ...
  out.subH2 = subH2s.map((h) => ({
    t: TXT(h),
    rect: R(h),
    style: S(h, [...FONT, "margin"]),
    lines: LINES(h),
  }));
  out.intro = {
    t: (TXT(introH3) || "").slice(0, 40),
    rect: R(introH3),
    style: S(introH3, [...FONT, "margin", "maxWidth"]),
    lines: LINES(introH3),
  };

  // --- grid
  const gridSec = live
    ? q("section.team-grid-section")
    : q("section.team-grid-section");
  out.gridSec = R(gridSec);
  out.gridSecStyle = S(gridSec, [
    "padding",
    "maxWidth",
    "display",
    "flexWrap",
  ...
  out.gridInnerStyle = S(gridInner, [
    "padding",
    "maxWidth",
    "display",
    "flexWrap",
  ...
  const contentWidth = live
    ? q("section.team-grid-section .content-width")
    : null;
  ...
  if (c0) {
    const headshot = live
      ? q("img.team-grid-headshot", c0)
      : q("img[class*='rounded-full']", c0);
    const name = q("h5", c0);
    const role = live ? q("h6.h7", c0) : qa("h6", c0)[0];
    const bio = live ? q("p.team-teaser", c0) : q("p", c0);
    const beachImg = live
      ? q("img.team-grid-beach", c0)
      : q("img[class*='absolute']", c0);
    const beachName = live
      ? q("h6.team-beach-name", c0)
      : qa("h6", c0).slice(-1)[0];
    const rm = live
      ? q(".team-teasewr-read-more", c0)
      : [...c0.querySelectorAll("a")].find((a) =>
          /read more/i.test(a.textContent || ""),
        );
    const rmLink = live ? (rm ? rm.parentElement : null) : rm;
    const arrow = live
      ? q("img.read-more-arrow", c0)
      : rm
        ? q("span", rm)
        : null;
    ...
    out.headshotParent = {
      rect: R(headshot ? headshot.parentElement : null),
      style: S(headshot ? headshot.parentElement : null, [
        "position",
        "display",
        "width",
        "height",
    ...
    out.name = {
      t: TXT(name),
      rect: R(name),
      style: S(name, [...FONT, "margin"]),
      lines: LINES(name),
    };
    out.role = {
      t: TXT(role),
      rect: R(role),
      style: S(role, [...FONT, "margin"]),
      lines: LINES(role),
    };
    out.bio = {
      t: TXT(bio),
      rect: R(bio),
      style: S(bio, [...FONT, "margin", "height", "overflow", "padding"]),
      lines: LINES(bio),
    };
    out.beachImg = {
      rect: R(beachImg),
    ...
    out.beachName = {
      t: TXT(beachName),
      rect: R(beachName),
      style: S(beachName, [...FONT, "position", "bottom", "left", "margin"]),
      lines: LINES(beachName),
    };
    out.rm = {
      t: TXT(rm),
      rect: R(rm),
      style: S(rm, [...FONT, "margin", "display", "gap"]),
      lines: LINES(rm),
    };
    out.rmLink = {
      rect: R(rmLink),
      style: S(rmLink, ["display", "gap", "margin", "alignItems"]),
    };
    out.arrow = {
      rect: R(arrow),
      style: S(arrow, [
      ...
      src: arrow
        ? arrow.getAttribute("src") || arrow.className.toString().slice(0, 120)
        : null,
    };
    out.card0Html = c0.outerHTML
  ...
  out.cta = {
    t: TXT(cta),
    rect: R(cta),
    style: S(cta, [...FONT, "margin", "padding"]),
    lines: LINES(cta),
  };
  out.ctaParent = {
    rect: R(cta ? cta.parentElement : null),
    style: S(cta ? cta.parentElement : null, ["padding", "margin", "maxWidth"]),
  };
  // the section that owns the CTA
  let ctaSec = cta;
  while (ctaSec && ctaSec.tagName !== "SECTION" && ctaSec.tagName !== "FOOTER")
    ctaSec = ctaSec.parentElement;
  out.ctaSec = {
    cls: ctaSec ? ctaSec.className.toString().slice(0, 100) : null,
    rect: R(ctaSec),
    style: S(ctaSec, ["padding", "margin"]),
  };
...
fs.writeFileSync(
  "/Users/tuckerlemos/Documents/GitHub/beachfront-dentistry/matching/otv-main.json",
