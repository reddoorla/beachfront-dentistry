...
try {
  for (const [tag, base, path, bodyHead] of [
    ...
    const info = await p.evaluate((bodyHead) => {
      const Y = (e) =>
        e ? Math.round(e.getBoundingClientRect().top + scrollY) : null;
      const B = (e) =>
        e ? Math.round(e.getBoundingClientRect().bottom + scrollY) : null;
      const cta = [...document.querySelectorAll("h1,h2,h3")].find((e) =>
        /Ready for great/.test(e.textContent || ""),
      );
      const ctaTop = Y(cta);
      // lede = big cyan text-body element (h5/p) in upper page
      const lede = [...document.querySelectorAll("h4,h5,p")].find((e) => {
        const c = getComputedStyle(e);
        return (
          parseFloat(c.fontSize) >= 28 &&
          /18, ?158, ?204/.test(c.color) &&
          Y(e) > 400 &&
          Y(e) < ctaTop
        );
      });
      // body first heading
      const bh = [...document.querySelectorAll("h2,h3,h4,h5,h6,strong")].find(
        (e) => (e.textContent || "").trim().startsWith(bodyHead),
      );
      // last dark body paragraph ABOVE the cta
      const darks = [...document.querySelectorAll("p")].filter((e) => {
        const c = getComputedStyle(e);
        return (
