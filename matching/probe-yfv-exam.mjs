...
async function main() {
  ...
  try {
    ...
    await p
      .goto(LIVE, { waitUntil: "networkidle", timeout: 60000 })
      .catch(() => {});
    ...
    const out = await p.evaluate(() => {
      ...
      r.strongs = [...exam.querySelectorAll("strong,b")].map((s) =>
        s.textContent.trim(),
      );
      r.paras = [...exam.querySelectorAll("p")].map((p) =>
        p.textContent.trim().replace(/\s+/g, " ").slice(0, 140),
      );
      r.hasImg = exam.querySelectorAll("img").length;
      // toc cards text
      const toc = document.querySelector(".fv-toc-section");
      r.tocCards = [...toc.querySelectorAll("a")]
        .map((a) => a.textContent.trim().replace(/\s+/g, " ").slice(0, 60))
        .filter((t) => t.length > 1);
      // office tour captions
      const tour = document.querySelector(".fv-virtual-tour-section");
      r.tourCaptions = [
        ...tour.querySelectorAll('figcaption,[class*="caption"],.w-slide div'),
      ]
        .slice(0, 3)
        .map((c) => c.textContent.trim().slice(0, 50));
      r.tourHeadingHTML = tour
        .querySelector("h1,h2,h3")
        ?.outerHTML.slice(0, 120);
      // team section names count
      const team = document.querySelector(".fv-meet-our-team-section");
      r.teamHeadingHTML = team
        .querySelector("h1,h2,h3")
        ?.outerHTML.slice(0, 120);
