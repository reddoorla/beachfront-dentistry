...
for (const [tag, url] of [
  ...
  try {
    const p = await b.newPage({ viewport: { width: 650, height: 1000 } });
    ...
    const r = await p.evaluate(() => {
      const el = [
        ...document.querySelectorAll("h1,h2,h3,h4,h5,h6,a,div,p,span"),
      ].find(
        (e) =>
          (e.textContent || "")
            .replace(/\s+/g, " ")
            .trim()
            .toLowerCase()
            .startsWith("have a complete exam") &&
      ...
      for (let i = 0; i < 5 && anc; i++) {
        ...
        chain.push(
          `${anc.tagName}.${(anc.className || "").toString().slice(0, 20)} w=${Math.round(ar.width)}`,
        );
        anc = anc.parentElement;
      }
      return {
        titleW: Math.round(er.width),
        titleLines: Math.round(er.height / 26),
        chain,
      };
    });
