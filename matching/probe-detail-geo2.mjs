...
try {
  ...
  for (const vw of [390, 768, 1440]) {
    ...
    await p.goto(O + "/services/dental-exams", {
    ...
    const info = await p.evaluate(() => {
      ...
      const title = [...document.querySelectorAll("h1,h2,h3")].find(
        (e) =>
          parseFloat(getComputedStyle(e).fontSize) > 60 &&
          e.getBoundingClientRect().top + scrollY > 300,
      );
      const titleTop = title
        ? Math.round(title.getBoundingClientRect().top + scrollY)
        : null;
      const fiji = document.querySelector(".fiji-section, [class*='fiji']");
      const fijiH = fiji
        ? Math.round(fiji.getBoundingClientRect().height)
        : null;
      const hasFijiLabel = /FIJI/i.test(document.body.textContent || "");
      const cta = [...document.querySelectorAll("section,div")].find(
        (e) =>
          /Ready for great dental health/.test(e.textContent || "") &&
  ...
  for (const [tag, url] of [
    ["live", O + "/team-members/dr-robert-quan"],
