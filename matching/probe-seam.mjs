    ...
    try {
      ...
      const r = await p.evaluate((sel) => {
        ...
        for (let i = 0; i < 8 && a; i++) {
          ...
          if (
            /gradient|rgb\(1?[0-9]?[0-9],\s*1[0-9][0-9]/.test(bg) &&
        ...
        return {
          toothTop: Math.round(tr.top + sy),
          toothCenter: Math.round(tr.top + tr.height / 2 + sy),
          toothW: Math.round(tr.width),
          bandTop: br ? Math.round(br.top + sy) : null,
          bandTag: band
            ? band.tagName +
              "." +
              (band.className || "")
            ...
            : null,
          centerMinusBand: br
            ? Math.round(tr.top + tr.height / 2 - br.top)
            : null,
        };
      }, sel);
