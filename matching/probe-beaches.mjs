...
try {
  ...
  const cards = await page.evaluate(() => {
    ...
    return items.map((c) => {
      ...
      const roleEl = c.querySelector(
        'h6.text-align-center, h6.h7, [class*="h7"]',
      );
      ...
      const headshot = c.querySelector(
        '.team-grid-headshot, img[class*="headshot"]',
      );
      const readmore = [...c.querySelectorAll("a")].find((a) =>
        /read more/i.test(a.innerText || ""),
      );
      return {
        ...
        beachSrcset: beachImg
          ? (beachImg.getAttribute("srcset") || "")
              .split(",")
              .map((s) => s.trim())
              .slice(-1)[0]
          : null,
