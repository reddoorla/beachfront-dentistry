...
try {
  for (const [name,url] of [["live","https://www.beachfrontdentistry.com/"],["ours","http://localhost:5173/dev/match/home"]]) {
    ...
    const r = await p.evaluate(() => {
      const bx=(el,l)=>{if(!el)return `${l}: MISSING`;const b=el.getBoundingClientRect();const cs=getComputedStyle(el);
      ...
      const secs=[...document.querySelectorAll("body section, body header, body nav, body main > *")].slice(0,12);
      return secs.map((s,i)=>bx(s,`${i} ${s.tagName.toLowerCase()}.${String(s.className).split(" ").slice(0,2).join(".")}`)).join("\n  ");
    });
    console.log(`== ${name} @${VW}\n  ${r}`);
    await p.close();
  }
