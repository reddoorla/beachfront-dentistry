...
async function probe(page) {
  return await page.evaluate((tupleStr) => {
    ...
    if (cards[0]) {
      ...
      out.card0imgs = imgs.map((i) => ({
        ...tuple(i),
        src: (i.getAttribute("src") || "").slice(-60),
        nat: i.naturalWidth + "x" + i.naturalHeight,
      }));
      out.card0texts = [...c0.querySelectorAll("h1,h2,h3,h4,h5,h6,p,a")].map(
        (e) => ({ ...tuple(e) }),
      );
    }
    // grid container
    const grid = cards[0]
      ? cards[0].closest('.w-row, [class*="w-dyn-items"], [class*="row"]')
      : null;
    out.grid = tuple(grid);
    if (grid)
      out.gridCS = {
        ...
        gtc: getComputedStyle(grid).gridTemplateColumns,
