...
const probe = () => {
  ...
  const h2 = isLive
    ? document.querySelector(".our-team-subtitle-section h2")
    : document.querySelectorAll(
        '[data-slice-variation="subpage"] ~ section h2, section h2',
      )[1];
  const sub = isLive
    ? document.querySelector(".our-team-subtitle-section")
    : null;
  const hh = isLive
  ...
  const cs = (e) =>
    e
      ? (() => {
          ...
          return s.fontSize + "/" + s.lineHeight + " " + s.textAlign;
        })()
      : null;
  const hs = isLive
    ? document.querySelector(".team-grid-headshot")
