...
const DIR = "/Users/tuckerlemos/Documents/GitHub/beachfront-dentistry/matching/states";
...
try {
  ...
  for (const vw of [390, 1440]) {
    ...
    try {
      ...
      rec(`cand menu@${vw}`, open && closed && links >= 4, `open=${open} links=${links} closedAfter=${closed}`);
    } catch (e) {
  ...
  // 2) READ REVIEWS expander @1440
  {
    ...
    try {
      ...
      const clip = { x: Math.max(0, box.x - 40), y: Math.max(0, box.y - 20), width: 500, height: 200 };
      await p.screenshot({ path: `${DIR}/cand-readreviews-1440-closed.png`, clip });
      ...
      await p.screenshot({ path: `${DIR}/cand-readreviews-1440-open.png`, clip });
      rec("cand readReviews@1440", before === "false" && after === "true" && Number(rowOpacity) > 0.9,
  ...
  // 3) REVIEW SLIDER @1440 (Next advances slide)
  {
    ...
    try {
      ...
      const readIdx = async () =>
        p.evaluate(() => {
          const lr = [...document.querySelectorAll(".sr-only")].map((e) => e.textContent.trim()).find((t) => /slide/i.test(t));
          return lr ?? null;
        });
      const carousel = next.locator("xpath=ancestor::*[contains(@aria-roledescription,'carousel') or @aria-label][1]");
      const cbox = await (await carousel.first().boundingBox().catch(() => null)) || await next.boundingBox();
      const clip = { x: Math.max(0, cbox.x - 20), y: Math.max(0, cbox.y - 260), width: Math.min(1400, cbox.width + 40 || 900), height: 620 };
      ...
      rec("cand reviewSlider@1440", !!before && !!after && before !== after, `"${before}" -> "${after}"`);
    } catch (e) {
  ...
  // 4) QA ACCORDION @834 (first card expands)
  {
    ...
    try {
      ...
      const clip = { x: Math.max(0, box.x - 10), y: Math.max(0, box.y - 10), width: Math.min(834, box.width + 20), height: 460 };
      ...
      const after = await nowExpand.getAttribute("aria-expanded").catch(() => null);
      const clip2 = { x: clip.x, y: Math.max(0, box.y - 120), width: clip.width, height: 480 };
      await p.screenshot({ path: `${DIR}/cand-qa-834-open.png`, clip: clip2 });
      rec("cand qaAccordion@834", before === "false" && after === "true", `aria ${before}->${after}`);
    } catch (e) {
  ...
  // 5) HOVER states @1440 (pill bg + Read Reviews opacity change)
  {
    ...
    try {
      ...
      rec("cand hover: Request Appointment pill", bgBefore !== bgAfter, `bg ${bgBefore} -> ${bgAfter}`);
      ...
      rec("cand hover: Read Reviews", opBefore !== opAfter, `opacity ${opBefore} -> ${opAfter}`);
    } catch (e) {
  ...
  // Live menu @1440 (Webflow .menu-button opens .dropdown-modal-ish)
  {
    ...
    try {
      ...
      const trigger = p.locator('.menu-button, [class*="menu-button"], .w-nav-button').first();
  ...
  // Live review slider @1440 (advance via arrow)
  {
    ...
    try {
      await p.goto(LIVE, { waitUntil: "networkidle", timeout: 60000 });
      await settle(p);
      const arrow = p.locator('.w-slider-arrow-right, [class*="slider-arrow"]').first();
      await arrow.scrollIntoViewIfNeeded();
      const box = await arrow.boundingBox();
      const clip = { x: 0, y: Math.max(0, box.y - 260), width: 1440, height: 620 };
