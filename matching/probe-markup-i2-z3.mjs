...
const span = await p.evaluate(() => {
  ...
  return { start: Math.max(0, f.top + scrollY - 700), end: l.bottom + scrollY - 200, n: cards.length };
});
  ...
  for (const [name, sel] of Object.entries(SEL)) {
    const rect = await p.evaluate((s) => {
      ...
      return {
        ...
        width: Math.min(Math.round(r.width), 1440 - Math.max(0, Math.round(r.left))),
        height: Math.min(Math.round(r.height), 900 - Math.max(0, Math.round(r.top))),
      };
    }, sel);
    ...
    const over = await p.evaluate(({ r }) => {
      const el = document.elementFromPoint(r.x + r.width / 2, r.y + r.height / 2);
      if (!el) return null;
      return el.tagName.toLowerCase() + (typeof el.className === "string" && el.className ? "." + el.className.trim().split(/\s+/).slice(0, 3).join(".") : "");
    }, { r: rect });

    await p.evaluate((s) => {
      const e = document.querySelector(s);
      const w = e.closest("[class*='pointer-events-none']") || e;
      w.dataset.zsave = w.style.zIndex; w.style.zIndex = "99999";
    }, sel);
    ...
    for (let i = 0; i < A.data.length; i += 4)
      if (Math.abs(A.data[i]-B.data[i])>6 || Math.abs(A.data[i+1]-B.data[i+1])>6 || Math.abs(A.data[i+2]-B.data[i+2])>6) changed++;
    const pct = +((changed / (A.width * A.height)) * 100).toFixed(1);
    if (pct > 1) hits.push({ scrollY: y, which: name, pctOccluded: pct, paintedOverBy: over, rect });
  }
}
await ctx.close(); await b.close();
console.log(JSON.stringify({ span, sweptEvery: 120, occlusions: hits }, null, 1));
