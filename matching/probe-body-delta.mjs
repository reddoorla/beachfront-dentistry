...
try {
  for (const [tag, base] of [
    ...
    const info = await p.evaluate(() => {
      const bodyish = [...document.querySelectorAll("div,section")].filter(
        (e) => /At Beachfront Dentistry/.test(e.textContent || ""),
      );
      // narrowest wrapper that contains the body text
      const wrap = bodyish.sort(
        (a, z) => (a.textContent || "").length - (z.textContent || "").length,
      )[0];
      const paras = wrap ? [...wrap.querySelectorAll("p")] : [];
      const heads = wrap
        ? [...wrap.querySelectorAll("h1,h2,h3,h4,h5,h6")].map(
            (h) =>
              `${h.tagName}:${getComputedStyle(h).fontSize} "${h.textContent.trim().slice(0, 30)}"`,
          )
        : [];
      ...
      return {
        ...
        imgsEmbeds: imgs,
        firstPara: paras[0]
          ? `${getComputedStyle(paras[0]).fontSize}/${getComputedStyle(paras[0]).lineHeight} w=${Math.round(paras[0].getBoundingClientRect().width)}`
          : null,
      };
    });
