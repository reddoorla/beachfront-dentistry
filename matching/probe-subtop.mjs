  ...
  for (const pg of PAGES) {
    ...
    const info = await p.evaluate(() => {
      ...
      const all = [...document.querySelectorAll("body *")].filter((el) => {
        const b = el.getBoundingClientRect();
        return b.top < 700 && b.top >= 0 && b.width > 600 && b.height > 80 && !el.closest("nav");
      });
      ...
      const firstText = [...document.querySelectorAll("h1,h2,h3,h4,p,div")]
        ...
        .slice(0, 8)
        .map((el) => `${el.tagName}.${el.className.slice(0, 30)} @y${Math.round(el.getBoundingClientRect().top)} "${el.textContent.trim().slice(0, 35)}" — ${tuple(el)}`);
      // the top hero section box: the outermost section whose top<=0-ish
      const heroSection = document.querySelector("body main > *, body > div > section, section");
      return {
        firstText,
        heroSectionCls: heroSection?.className,
        heroSectionRect: heroSection ? r(heroSection) : null,
        heroBg: heroSection ? getComputedStyle(heroSection).backgroundColor : null,
        heroBgImg: heroSection ? getComputedStyle(heroSection).backgroundImage.slice(0, 70) : null,
      };
    });
    console.log(`\n===== ${pg} =====`);
