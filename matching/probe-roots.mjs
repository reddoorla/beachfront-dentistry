...
const ANCHORS = ["Path to Oral Health", "Comfort", "Paul K.", "Fiji Islands", "Read Reviews", "Serving the South Bay", "Finally have a dentist"];
...
try {
  for (const w of WIDTHS) {
    ...
    for (let y = 0; y < h0; y += 300) { await p.evaluate((yy) => window.scrollTo(0, yy), y); await p.waitForTimeout(60); }
    await p.waitForTimeout(200);
    const out = await p.evaluate((anchors) => {
      const root = getComputedStyle(document.documentElement).fontSize;
      const body = getComputedStyle(document.body).fontSize;
      const rows = anchors.map((a) => {
        const el = [...document.querySelectorAll("h1,h2,h3,h4,h5,h6,p,span,a,div,strong,li")]
          .find((e) => (e.textContent || "").replace(/\s+/g, " ").trim().toLowerCase().startsWith(a.toLowerCase())
