...
try {
  for (const [name,url,sel] of [
    ...
    ["ours","http://localhost:5173/dev/match/home",'[data-section-layout="services"]'],
  ]) {
    ...
    const r = await p.evaluate((sel) => {
      ...
      const walk=(el,d)=>{ if(d>5) return;
        for(const c of el.children){ const r=c.getBoundingClientRect(); if(r.height<1) continue;
          rows.push(`${"  ".repeat(d)}${c.tagName.toLowerCase()}.${String(c.className).split(" ").slice(0,2).join(".").slice(0,40)} ${bx(c)} "${(c.textContent||"").replace(/\s+/g," ").trim().slice(0,32)}"`);
          walk(c,d+1);} };
      walk(root,1);
      return rows.join("\n  ");
    }, sel);
