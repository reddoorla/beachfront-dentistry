...
async function measure(p) {
  return p.evaluate(() => {
    ...
    const q = (needle, tags = "h1,h2,h3,h4,h5,h6,p,a,span") =>
      [...document.querySelectorAll(tags)].find((e) =>
        norm(e.textContent).toLowerCase().startsWith(needle),
      );
    const info = (el) => {
      if (!el) return null;
  ...
  for (const vw of VWS) {
    ...
    for (const [k, url] of Object.entries(TARGETS)) {
      ...
      try {
        ...
        const f = (o) =>
          o ? `fs=${o.fs} ${o.w}x${o.h} lines=${o.lines}` : "—";
        console.log(
          `  ${k}: heading[${f(m.heading)}]  stepTitle[${f(m.stepTitle)}]  eyebrow[${f(m.eyebrow)}]`,
        );
      } catch (e) {
