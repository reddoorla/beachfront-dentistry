...
try {
  for (const vw of [1440, 834, 390]) {
    ...
    const r = await p.evaluate(() => {
      ...
      const h=[...document.querySelectorAll("h1,h2,h3")].find(e=>/^Finally have a dentist/.test(e.textContent.replace(/\s+/g," ").trim()));
      ...
      labels.forEach((L)=>{
        ...
        let card=sp; for(let i=0;i<8&&card.parentElement;i++){card=card.parentElement; const cs=getComputedStyle(card);
          if(card.getBoundingClientRect().width>200&&parseFloat(cs.borderTopLeftRadius)>10) break;}
        const bar=sp.closest("button,div[class*='bottom-0']");
        const txt=card.querySelector("[id*='card-']");
        out.push(`${L} card ${bx(card)} | bar ${bx(bar)} h=${bar?Math.round(bar.getBoundingClientRect().height):"-"} | text ${bx(txt)} | labelFs ${getComputedStyle(sp).fontSize}/${getComputedStyle(sp).lineHeight}`);
      });
      return out.join("\n  ");
    });
    console.log(`== ours @${vw}\n  ${r}`);
