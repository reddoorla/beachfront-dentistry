...
try {
  for (const vw of [1440, 834, 390]) {
    ...
    const r = await p.evaluate(() => {
      ...
      const eyebrow = [...document.querySelectorAll("h6")].find((e) =>
        /MEET YOUR TEAM/i.test(e.textContent),
      );
      return [
        box(holder, "holder"),
        box(slider, "slider"),
        ...
        eyebrow
          ? box(eyebrow, "eyebrow") +
            " mb=" +
            getComputedStyle(eyebrow).marginBottom +
            " fs=" +
            getComputedStyle(eyebrow).fontSize +
            "/" +
            getComputedStyle(eyebrow).lineHeight +
            " ls=" +
            getComputedStyle(eyebrow).letterSpacing +
            " w=" +
            getComputedStyle(eyebrow).fontWeight +
            " tt=" +
            getComputedStyle(eyebrow).textTransform
          : "eyebrow MISSING",
        "count=" + document.querySelectorAll(".heads").length,
      ].join("\n  ");
    });
    console.log(`== live @${vw}\n  ${r}`);
    await p.close();
