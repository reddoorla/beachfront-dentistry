...
const grab = () => {
  ...
  const o = [];
  const hdr = document.querySelector("section.header") || document.querySelector("nav");
  o.push(`HDR ${rr(hdr)} h=${hdr ? getComputedStyle(hdr).height : "-"}`);
  if (hdr) {
    const imgs = [...hdr.querySelectorAll("img,svg")];
    imgs.slice(0, 4).forEach((i, n) => o.push(`  media${n} ${i.tagName} ${rr(i)} src=${(i.getAttribute("src") || "").split("/").pop()}`));
    const as = [...hdr.querySelectorAll("a,button")].filter((a) => a.offsetHeight > 0);
    as.slice(0, 6).forEach((a, n) => o.push(`  link${n} ${rr(a)} "${(a.innerText || "").trim().slice(0, 24)}"`));
  }
  return o.join("\n");
};
...
const run = async () => {
  ...
  try {
    ...
    const dims = await page.evaluate(async () => {
      const load = (u) => new Promise((res) => { const i = new Image(); i.onload = () => res(`${i.naturalWidth}x${i.naturalHeight}`); i.onerror = () => res("ERR"); i.src = u; });
      return {
        redondo: await load("https://cdn.prod.website-files.com/64af3f93339537d6b661b556/64af4ef42e7d98b2fdb91769_beach-in-beautiful-morning-light-at-redondo-beach-75226436.jpeg"),
      };
    });
