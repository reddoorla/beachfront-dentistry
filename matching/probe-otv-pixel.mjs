  ...
  for (const [name, url] of [
    ...
    const box = await page.evaluate((live) => {
      const c = document.querySelector(
        live ? ".team-list-item" : "article.team-list-item",
      );
      const r = c.getBoundingClientRect();
      return {
        x: r.left + window.scrollX,
        y: r.top + window.scrollY - 110,
        width: r.width,
        height: r.height + 120,
      };
    }, name === "live");
    await page.screenshot({
      path: `${DIR}/otv-card-${name}.png`,
      clip: box,
      fullPage: true,
    });
    ...
    const arrowBox = await page.evaluate((live) => {
      ...
      const a = live
        ? c.querySelector("img.read-more-arrow")
        : c.querySelector("a span[aria-hidden]");
