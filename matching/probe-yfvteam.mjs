...
try {
  for (const vw of [1440,834,390]) {
    ...
    await p.goto("https://www.beachfrontdentistry.com/your-first-visit", { waitUntil: "networkidle", timeout: 60000 });
    await settle(p);
    const r = await p.evaluate(() => {
      ...
      return [`section ${bx(sec)}`,
        ...
        ...cards.slice(0,3).map((c,i)=>`card${i} ${bx(c)} cls=${String(c.className).slice(0,60)}`)].join("\n  ");
    });
    console.log(`== live @${vw}\n  ${r}`);
    await p.close();
  }
