...
try {
  ...
  const inv = await p.evaluate(() => {
    ...
    return {
      tourArrows:
        tour?.querySelectorAll('button[aria-label*="slide"]').length ?? 0,
      tourDots:
        tour?.querySelectorAll('button[aria-label^="Go to slide"]').length ?? 0,
      teamArrows:
        team?.querySelectorAll('button[aria-label*="slide"]').length ?? 0,
      appointmentLinks: [
        ...document.querySelectorAll('a[href="#appointment"]'),
      ].map((a) => a.textContent.trim()),
      navCardHrefs: [...document.querySelectorAll(".visit-list-item")].map(
  ...
  const nextBtn = await p.$(
    '[data-slice-variation="photos"] button[aria-label="Next slide"]',
  );
  const before = await p.$eval(
    '[data-slice-variation="photos"] .flex.transition-transform',
    (e) => e.style.transform,
  );
  ...
  console.log(
    JSON.stringify(
      { ...inv, tourAdvances: before !== after, before, after },
      null,
      2,
    ),
  );
