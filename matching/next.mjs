...
const TOTALS = { home: 27, yfv: 24, "our-team": 15, services: 15, atd: 15, contact: 12, team: 15, svc: 15, qa: 15 };
...
for (const d of readdirSync(DIR).filter((d) => d.startsWith("out-"))) {
  ...
  if ((meta.mask?.length ?? 0) > 0 || meta.neutralizeMedia || meta.maskPhotos || meta.truncated) continue;
...
for (const [page, { dir, report }] of latest) {
  ...
  for (const f of fails) {
    ...
    if (floor) { floorTotal++; continue; }
    const ack = ACCEPTED.find((a) => a.match(f));
    if (ack) { accepted.push({ page, vw: f.viewport, label: f.label, why: ack.why }); continue; }
    openTotal++;
    rows.push({
      page, vw: f.viewport, label: f.label,
      mm: f.mismatchFraction, dh: f.heightDeltaFraction ?? 0, dir,
    });
  }
}

const scored = [...latest.entries()]
  .map(([p, v]) => ({ p, pass: v.report.regions.filter((r) => r.pass).length, total: TOTALS[p] }))
...
console.log(scored.map((s) => `  ${s.p.padEnd(9)} ${String(s.pass).padStart(2)}/${s.total}`).join("\n"));
...
if (!rows.length) {
  ...
  console.log("Backlog is empty — Phases 5 (states) and 6 (adversarial review) are what is left.");
  process.exit(0);
}
...
rows.sort((a, b) =>
  (a.page === worst ? -1 : 0) - (b.page === worst ? -1 : 0) ||
  b.mm - a.mm);
...
for (const r of rows.filter((r) => r.page === worst)) {
  ...
  console.log(`  @${String(r.vw).padEnd(5)} ${r.label.slice(0, 44).padEnd(45)} ${why.join(" + ")}`);
}
console.log(`\nBefore treating any of these as geometry:`);
console.log(`  node matching/probe-anchor-parity.mjs ${worst}   # is the gate cutting comparably?`);
console.log(`  node matching/strikes.mjs ${worst}               # has it stalled? then change the MODEL`);
