...
async function probe(page) {
  return await page.evaluate((cssTupleStr) => {
    ...
    if (hero)
      out.heroHeading = cssTuple(
        hero.querySelector('h1,h2,.heading,[class*="heading"]'),
      );
    ...
    if (grid) {
      ...
      out.gridExtra = {
        padT: gc.paddingTop,
        padR: gc.paddingRight,
        padB: gc.paddingBottom,
        padL: gc.paddingLeft,
        justify: gc.justifyContent,
        gtr: gc.gridTemplateRows,
      };
    ...
    if (blocks[0]) {
      ...
      if (top) {
        out.cardHeading = cssTuple(
          top.querySelector('h1,h2,h3,h4,h5,[class*="heading"]'),
        );
        const paras = [
          ...top.querySelectorAll('p,[class*="paragraph"],[class*="text"]'),
        ];
        out.cardIntro = cssTuple(
          paras.find((p) => (p.innerText || "").trim().length > 20) || paras[0],
        );
      }
      if (bottom) {
        ...
        out.linkLabel = cssTuple(
          links[0] && links[0].querySelector('h6,[class*="h6"],h1,h2,h3,h4,h5'),
        );
        const halves = [
          ...bottom.querySelectorAll('._w-half,[class*="w-half"]'),
        ];
        ...
        out.linkYs = links.slice(0, 6).map((a) => {
      ...
      const teef = b.querySelector(
        '.service-block-teef,img[class*="teef"],img',
      );
      out.teef = cssTuple(teef);
      if (teef) {
        out.teefSrc = teef.getAttribute("src");
        out.teefNatural = { nw: teef.naturalWidth, nh: teef.naturalHeight };
        out.teefSizes = teef.getAttribute("sizes");
        out.teefSrcset = (teef.getAttribute("srcset") || "").slice(0, 120);
      }
    }

    out.blocksSummary = blocks.map((b) => {
      const h = b.querySelector(
        '.h-60pc h1,.h-60pc h2,.h-60pc h3,.h-60pc h4,.h-60pc h5,[class*="heading"]',
      );
      return {
        heading: h ? (h.innerText || "").trim() : "",
        links: b.querySelectorAll('.h-40pc a,[class*="h-40"] a').length,
      };
    });
    ...
    if (grid2) {
      ...
      const gridBand =
        grid2.closest('section,[class*="section"],[class*="band"]') ||
        grid2.parentElement;
      out.gridBand = cssTuple(gridBand);
      out.aboveGridBand = cssTuple(
        gridBand ? gridBand.previousElementSibling : null,
      );
    }

    const heads = [...document.querySelectorAll("h1,h2,h3,h4")];
    const cta = heads.find((h) =>
      /ready for great dental health/i.test(h.innerText || ""),
    );
    out.cta = cssTuple(cta);
    if (cta)
      out.ctaBand = cssTuple(
        cta.closest('section,[class*="section"],[class*="band"]'),
      );
