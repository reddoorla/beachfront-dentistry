...
const fn = () => {
  ...
  put(
    "L.review-viewport",
    document.querySelector(".review-slider-holder-viewport"),
  );
  ...
  put(
    "L.ssb-content",
    document.querySelector(".home-ssb-section .content-width"),
  );
  put("L.services-section", document.querySelector(".home-services-section"));
  put(
    "L.services-content",
    document.querySelector(".home-services-section .content-width"),
  );
  ...
  put(
    "L.hm-content",
    document.querySelector(".home-healthy-mouth-section .content-width"),
  );
  ...
  put(
    "L.myt-content",
    document.querySelector(".home-meet-your-team-section .content-width"),
  );
  ...
  if (foot) {
    const kids = [...foot.querySelectorAll("*")].filter((e) => {
      ...
      return (
        /rights reserved|privacy|sitemap|©|20\d\d beachfront/i.test(t) &&
        e.children.length === 0
      );
    });
    out["L.footer-legal"] = kids
      .map((e) => `${abs(e)} "${e.textContent.trim().slice(0, 26)}"`)
      .join(" | ");
    out["L.footer-legal-parent"] = kids[0]
      ? box(kids[0].parentElement)
      : "none";
  }
  // ---- generic (both sides) ----
  const firstQaImg = document.querySelector(
    'img[src*="couple_running_in_beach"]',
  );
  if (firstQaImg) {
    let e = firstQaImg,
      chain = [];
    for (let i = 0; i < 4 && e; i++) {
      chain.push(
        `${e.tagName}.${(e.className || "").toString().slice(0, 30)} ${abs(e)}`,
      );
      e = e.parentElement;
    }
  ...
  if (bookStep) {
    ...
    for (let i = 0; i < 4 && e; i++) {
      chain.push(
        `${e.tagName}.${(e.className || "").toString().slice(0, 34)} ${abs(e)}`,
      );
      e = e.parentElement;
    }
  ...
  const legal = [...document.querySelectorAll("*")].filter(
    (e) =>
      e.children.length === 0 &&
      /^(All Rights Reserved|Privacy Policy|Sitemap)$/i.test(
        (e.textContent || "").trim(),
      ),
  );
  out["G.legal"] = legal
    .map((e) => `${abs(e)} "${e.textContent.trim().slice(0, 22)}"`)
    .join(" | ");
  const badge = byText("h6,span,div", "01");
  const circ =
    document.querySelector(".qa-circle") ||
    [...document.querySelectorAll("span,h6")].find(
      (e) => (e.textContent || "").trim() === "01" && e.children.length === 0,
    );
  ...
  if (rp) {
    ...
    while (e && e !== document.body) {
      ...
      chain.push(
        `${e.tagName}.${(e.className || "").toString().slice(0, 34)} ${abs(e)} ov=${s.overflowX}/${s.overflowY}`,
      );
      e = e.parentElement;
    }
...
try {
  for (const [name, url] of [
    ...
    for (const [k, v] of Object.entries(r))
      if (v && v !== "MISSING") console.log(`${k}\t${v}`);
    await ctx.close();
  }
} finally {
  await browser.close();
