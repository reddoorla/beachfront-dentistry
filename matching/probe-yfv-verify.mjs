...
try {
  ...
  const dump = await p.evaluate(() => {
      ...
      const el = [...document.querySelectorAll("*")].find(
        (e) =>
          e.textContent
            ...
            .trim()
            .toLowerCase()
            .startsWith(text.toLowerCase()) && e.children.length < 3,
      );
      // climb to the section wrapper
      let s = el;
      while (s && s.parentElement && !/section|div/i.test(s.tagName))
        s = s.parentElement;
      return el;
    };
    const secWrap = (headingText) => {
      ...
      while (
        s &&
        !(s.className && /section/i.test(s.className)) &&
        s.parentElement
      )
        s = s.parentElement;
    ...
    const examImgs = exam
      ? [...exam.querySelectorAll("img")].map((i) =>
          i.src.split("/").pop().slice(0, 40),
        )
      : [];
    const examCols = exam
      ? getComputedStyle(exam).display +
        " cols=" +
        (exam.querySelector("[class*=col],[class*=w-col]") ? "yes" : "no")
      : "";
    ...
    const tocCards = toc
      ...
      : [];
    const tocArrows = toc
      ? [...toc.querySelectorAll("img")].map((i) => i.src.split("/").pop())
      : [];
    ...
    const slides = slider
      ? slider.querySelectorAll(".w-slide, [class*=slide]").length
      : 0;
    const dots = slider
      ? slider.querySelectorAll(".w-slider-dot, [class*=dot]").length
      : 0;
    const arrows = slider
      ? slider.querySelectorAll(
          ".w-slider-arrow-left, .w-slider-arrow-right, [class*=arrow]",
        ).length
      : 0;
    const slide0 = slider?.querySelector(".w-slide, [class*=slide]");
    const slideRect = slide0
      ? (() => {
          const r = slide0.getBoundingClientRect();
    ...
    const teamCards = teamSlider
      ? teamSlider.querySelectorAll("[class*=slide]").length
      : 0;
    const teamArrows = team
      ? [...team.querySelectorAll("img")]
          .map((i) => i.src.split("/").pop())
          .filter((s) => /arrow/i.test(s))
      : [];

    return {
      EXAM: {
        wrapperClass: exam?.className?.toString().slice(0, 60),
        display: examCols,
        hasMIN,
        hasStepNums,
        imgs: examImgs,
      ...
      TOC: {
        wrapperClass: toc?.className?.toString().slice(0, 60),
        cards: tocCards,
        arrows: tocArrows,
        rows: textRows(toc),
      },
      TOUR: {
        sliderClass: slider?.className?.toString().slice(0, 60),
        slides,
        dots,
        arrows,
        slide0: slideRect,
      ...
      TEAM: {
        sliderClass: teamSlider?.className?.toString().slice(0, 60),
        teamCards,
        teamArrows,
      },
    };
